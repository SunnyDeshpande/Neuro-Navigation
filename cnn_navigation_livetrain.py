...
model = load_model('/home/isera2/catkin_sunny/src/sunny_pioneer/models/modelgpufull1/batch0network')
...
def trainnetwork(image, vector, cmd_vels):		
    with graph.as_default():    
        model.train_on_batch([image, vector], cmd_vels, sample_weight=None, class_weight=None)
...
def callback(image_rgb, image_depth, odometry, local_plan, cmdvel):
	...
	outputcmd_vel.append(cmdvel.twist.linear.x)
	outputcmd_vel.append(cmdvel.twist.angular.z)
	outputcmd_vel = np.array(outputcmd_vel)
	outputcmd_vel = np.expand_dims(outputcmd_vel, axis=0)
	trainnetwork(inputimage, inputvector, outputcmd_vel)
	prediction = predictcmdvel(inputimage, inputvector)

	vel_msg.linear.x = prediction[0,0]
	vel_msg.linear.y = 0
	vel_msg.linear.z = 0
	vel_msg.angular.x = 0
	vel_msg.angular.y = 0
	vel_msg.angular.z = prediction[0,1]
...
rospy.init_node('cnn_navigation_livetrain', anonymous=True)
